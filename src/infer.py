...
parser.add_argument('-rtt', '--ratiothreetwo', help='ratio of three-gram to two-gram', default=0.05, required=False)
parser.add_argument('-rto', '--ratiotwoone', help='ratio of two-gram to one-gram', default=0.9, required=False)
parser.add_argument('-u', '--use_extra_p', help='use mixed three to two prob', default='no', required=False)
parser.add_argument('-p', '--parse', help='parse comma... or not', default='no', required=False)
...
while True:
    ...
    for i in range(len(input_pinyins)):
        ...
        if i == 1 or (i > 1 and args.three == 'no'):
        ...
        for word in pinyin2word[cur_pinyin]:
            dp[i][word] = math.inf
            for last_word in pinyin2word[last_pinyin]:
                for last_last_word in pinyin2word[last_last_pinyin]:
                    zero_flag = False
                    ...
                    # if last_last_word not in dp[i-2]:
                    #     continue

                    # if cur_score + dp[i-2][last_last_word] < dp[i][word]:
                    #     dp[i][word] = cur_score + dp[i-2][last_last_word]
                    #     path[i][word] = last_word
                    if last_word not in dp[i-1]:
                        continue
    ...
    final_words = ''
    min_score = math.inf
    for key, value in final_dp.items():
        if value < min_score:
            min_score = value
            final_words = key
    ...
    for i in range(len(input_pinyins)-1, -1, -1):
